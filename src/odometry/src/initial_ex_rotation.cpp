#include "../inc/initial_ex_rotation.h"
...
Matrix3d InitialEXRotation::solveRelativeR(const vector<pair<Vector3d, Vector3d>> &corres)
{
    ...
    if (corres.size() >= 9)
    {
        ...
        // 这里直接调用了OpenCV里面的函数来实现对极几何求解
        cv::Mat E = cv::findFundamentalMat(ll, rr);
        cv::Mat_<double> R1, R2, t1, t2;
        // 从本质矩阵中反解出对应的旋转矩阵，旋转矩阵和位移向量都有两种情况
                                          ...
                                          cv::Mat_<double> R, cv::Mat_<double> t)
{
    ...
    cv::triangulatePoints(P, P1, l, r, pointcloud); // 三角化反解出特征点的三维坐标
    int front_count = 0;
