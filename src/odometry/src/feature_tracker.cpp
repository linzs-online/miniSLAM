...
void FeatureTracker::IntrinsicParameter(){
    for (size_t i = 0; i < parameters->CAM_NAMES.size(); i++)
    {
        ROS_INFO("reading paramerter of camera %s", parameters->CAM_NAMES[i].c_str());
        camodocal::CameraPtr camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(parameters->CAM_NAMES[i]);
...
FeatureMap FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img0, const cv::Mat &_img1)
{
    ...
    if (prev_pts.size() > 0)
    {
        vector<uchar> status; // 记录成功追踪的特征点
        vector<float> err;
        cv::calcOpticalFlowPyrLK(prev_image, cur_image, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
    ...
    setMask(); // 屏蔽稳定的特征点附近的区域
    int insufficientPts_cnt = parameters->MAX_CNT - cur_pts.size();
    ...
    if (insufficientPts_cnt > 0)
    {
        ...
        cv::goodFeaturesToTrack(cur_image, n_pts, insufficientPts_cnt, 0.01, parameters->MIN_DIST, mask);
    }
    else
        n_pts.clear();
    // 把新提取的特征点存起来
    ...
    cur_un_pts = undistortedPts(cur_pts, m_camera[0]);  // 获得去畸变归一化坐标
...
{
    mask = cv::Mat(row, col, CV_8UC1, cv::Scalar(255)); // 先初始化一张全白的和原图一样大小的图片
    ...
    cur_pts.clear();
    ids.clear();  // 把当前帧的特征点 ID vector清零
    track_cnt.clear();
    ...
    {// 类似非极大值抑制算法，在追踪次数很大的点附近不再记录
        if (mask.at<uchar>(it.second.first) == 255)
        {
            ...
            cv::circle(mask, it.second.first, parameters->MIN_DIST, 0, -1);
        }
    }
}
...
vector<cv::Point2f> FeatureTracker::undistortedPts(vector<cv::Point2f> &pts, camodocal::CameraPtr cam)
{
    ...
    for (unsigned int i = 0; i < pts.size(); i++)
    {
        ...
        Eigen::Vector3d b;
        cam->liftProjective(a, b); //通过迭代法求出无畸变归一化坐标
